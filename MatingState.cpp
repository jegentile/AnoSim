/*
 *  MatingState.cpp
 *  Untitled
 *
 *  Created by James E. Gentile on 2/10/10.
 *  Copyright 2010 __MyCompanyName__. All rights reserved.
 *
 */

#include "MatingState.h"

MatingState::MatingState(){
	return;
}

MatingState::MatingState(int numOfOutputs,int hoursInState,AbmEnvironment* environment, AbmAgent* agent, string name){
	numOutputs_ =numOfOutputs;
	outputThresholds_= new float[numOutputs_];
	outputs_ = new AbmState*[numOutputs_];
	environment_ = environment;
	agent_ = agent;
	name_ = name;
	hoursIn_ = hoursInState;
	return;
}



AbmState* MatingState::update(){
	if(isTimeToLeave()){
		return outputs_[0];
	}
	return this;
}
AbmState* MatingState::enter(){
	return this;
}